<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Menus.aspx.cs" Inherits="Menus" Culture="auto" UICulture="auto" %>
...
<body style="background-color: #FFF !important">
    <div style="width: 100%;" id="content">
        ...
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 variaveis" id="divNew" runat="server" style="margin-bottom: 25px;">
            ...
            <div class='col-lg-4 col-md-4 col-sm-4 col-xs-4 line'>
                Ordem
                <input type='number' class='form-control' id='ordemNew' placeholder='Ordem' required='required' style='width: 100%; margin: auto;' value=''/>
            </div>
            <div class='col-lg-4 col-md-4 col-sm-4 col-xs-4 line'>
                Administrador
    ...
    <script type="text/javascript">
        ...
        function edit() {
            alertify.confirm('Menus', 'Tem a certeza que deseja atualizar a informação relativa ao menu?',
                function () {
                    if ($('#tituloEdit').val() == "") {
                    ...
                    if ($('#ordemEdit').val() == "") {
                        alertify.message('O campo Ordem é obrigatório!');
                        return;
                    }
                    ...
                    $.ajax({
                        ...
                        data: '{"operatorID":"' + $('#lbloperatorid').html() + '", "titulo":"' + $('#tituloEdit').val() + '", "pagina":"' + $('#paginaEdit').val()
                            ...
                            + '", "ordem":"' + $('#ordemEdit').val() + '", "menuID":"' + menuID + '"}',
                        contentType: "application/json; charset=utf-8",
                        dataType: "json",
                        success: function (res) {
                            if (res.d != null) {
                                if (parseInt(res.d) >= 0) {
                                    alertify.message('Menu atualizado com sucesso!');
                                    $('#search').val('');
                                    closeEdit();
                                    loadGrid();
                                }
                                else {
                                    alertify.message('Ocorreu um erro ao atualizar o Menu!');
                                }
                            }
                        }
                    });
        ...
        function remove() {
            alertify.confirm('Menus', 'Tem a certeza que deseja apagar o menu?',
                function () {
                    $.ajax({
                        type: "POST",
                        url: "Menus.aspx/deleteMenu",
                        data: '{"id":"' + menuID + '"}',
                        contentType: "application/json; charset=utf-8",
                        dataType: "json",
                        success: function (res) {
        ...
        function newMenu() {
            alertify.confirm('Menus', 'Tem a certeza que inserir a informação relativa ao menu?',
                function () {
                    if ($('#tituloNew').val() == "") {
                        alertify.message('O campo Título é obrigatório!');
                        return;
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            if (res.d != null) {
                                //$('#divNew').html(res.d);
                                if (parseInt(res.d) >= 0) {
                                    alertify.message('Menu criado com sucesso!');
                                    $('#search').val('');
                                    closeNew();
                                    loadGrid();
                                }
                                else {
                                    alertify.message('Ocorreu um erro ao criar o Menu!');
                                }
                            }
                        }
                    });
