<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Comments.aspx.cs" Inherits="Comments" Culture="auto" UICulture="auto" %>
...
<body style="background-color: #FFF !important">
    <div style="width: 100%;" id="content">
        ...
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" id="divTable">
            <div class="col-lg-9 col-md-9 col-sm-9 col-xs-9" style="margin-bottom: 10px;">
                ...
                <input type="image" src="img/icons/icon_search.png" onclick="loadTiposContrato();" style="float:right; width:auto; height:50px; margin:auto;" />
            </div>
            <div class="col-lg-3 col-md-3 col-sm-3 col-xs-3" style="margin-bottom:10px;">
                <input type="button" value="Apagar Selecionados" onclick="apagarComentarios();" style="float: right; background-color: #4282b5; width: 80%; height: 50px; font-size: 12pt; text-align: center; line-height: 50px; color: #FFFFFF; cursor: pointer; vertical-align: middle; border: none; padding: 0 5px; -moz-border-radius: 2px; -webkit-border-radius: 2px; border-radius: 2px;" />
            </div>
            <%--<div class="col-lg-2 col-md-2 col-sm-2 col-xs-2" style="margin-bottom:10px;">
                <input type="button" value="Atualizar" onclick="updateComments();" style="float: right; background-color: #4282b5; width: 80%; height: 50px; font-size: 12pt; text-align: center; line-height: 50px; color: #FFFFFF; cursor: pointer; vertical-align: middle; border: none; padding: 0 5px; -moz-border-radius: 2px; -webkit-border-radius: 2px; border-radius: 2px;" />
            </div>--%>
    ...
    <script type="text/javascript">
        ...
        function changeTo(pos) {
            if ($('#aprovado_' + pos.toString()).html() == "1") {
                ...
                $('#img_' + pos.toString()).attr('src', 'img/icons/not_approved.png');
            }
            else {
                $('#aprovado_' + pos.toString()).html('1');
                $('#img_' + pos.toString()).attr('src', 'img/icons/approved.png');
            }
        }

        function selectToDelete(pos) {
            if ($('#delete_img_' + pos.toString()).attr('src') == 'img/icons/not_approved.png') {
                $('#delete_img_' + pos.toString()).attr('src', 'img/icons/approved.png');
            }
            else {
                $('#delete_img_' + pos.toString()).attr('src', 'img/icons/not_approved.png');
            }
        }

        function changeApproval(id, i) {
            var aprovado;

            if ($('#approveComment' + i).attr('src') == 'img/icons/not_approved.png') {
                $('#approveComment' + i).attr('src', 'img/icons/approved.png');
                aprovado = '1';
            }
            else {
                $('#approveComment' + i).attr('src', 'img/icons/not_approved.png');
                aprovado = '0';
            }
            ...
            $.ajax({
                ...
                data: '{"artigo":"' + art + '", "id":"' + id + '", "aprovado":"' + aprovado + '"}',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (res) {
                    if (res.d != null) {
                        alertify.message('Comentários atualizados com sucesso!');
                        loadTiposContrato();
                    }
                }
            });
        ...
        function apagarComentarios() {
            alertify.confirm('Remover Comentários', 'Tem a certeza que deseja remover os comentários selecionados?', function () {
                deleteComments();
            }
        ...
        function deleteComments() {
            for (i = 0; i < parseInt($('#countElements').html()); i++) {
                if ($('#delete_img_' + i.toString()).attr('src') == 'img/icons/approved.png') {
                    var id = $('#id_' + i.toString()).html();
                    var art = $('#art_' + i.toString()).html();
                    deleteComentario(id, art);
                }
            }
        ...
        function deleteComentario(id, artigo) {
            $.ajax({
                    type: "POST",
                    url: "Comments.aspx/deleteComentario",
                    data: '{"id_operador":"' + $('#lbloperatorid').html() + '", "id":"' + id + '", "artigo":"' + artigo + '"}',
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function (res) {
                        if (res.d != null) {
