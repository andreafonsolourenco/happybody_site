<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Avisos.aspx.cs" Inherits="Avisos" Culture="auto" UICulture="auto" %>
...
<body style="background-color: #FFF !important">
    ...
    <script type="text/javascript">
        ...
        function saveAviso() {
            if($('#nrsocio').val().trim() == '') {
                alertify.message('O Nº de Sócio tem de ser preenchido!');
                return;
            }

            if($('#valor').val().trim() == '') {
                alertify.message('O Valor tem de ser preenchido!');
                return;
            }
            ...
            alertify.confirm('Avisos', 'Tem a certeza que deseja guardar o novo aviso?',
                function () {
                    $.ajax({
                        type: "POST",
                        url: "Avisos.aspx/guardaAviso",
                        data: '{"id_operador":"' + $('#lbloperatorid').html() + '", "nr_socio":"' + $('#nrsocio').val().trim()
                            + '", "valor":"' + $('#valor').val().trim() + '", "notas":"' + $('#notas').val().trim()
                            + '", "data":"' + $('#data').val().trim() + '", "entrada":"' + entrada + '"}',
                        contentType: "application/json; charset=utf-8",
                        dataType: "json",
                        success: function (res) {
                            if (res.d != null) {
                                //$('#search').val(res.d);
                                if(parseInt(res.d) > 0) {
                                    alertify.message('Novo aviso inserido com sucesso!');
                                    $('#search').val('');
                                    load();
                                }
                                else {
                                    alertify.message('Ocorreu um erro ao inserir um novo aviso!');
                                }
                            }
                        }
                    });
        ...
        function openAviso(id_aviso) {
            openPopup();

            $.ajax({
                type: "POST",
                url: "Avisos.aspx/loadAviso",
                ...
                success: function (res) {
                    if (res.d != null) {
                        $('#popup').html(res.d);
                        $('#dataedit').datepicker({ format: 'dd/mm/yyyy', changeYear: true, changeMonth: true, orientation: 'bottom' });
                    }
                }
            });
        }
        ...
        function deleteAviso(id_aviso) {
            alertify.confirm('Avisos', 'Tem a certeza que deseja apagar o aviso?',
                function () {
                    $.ajax({
                        type: "POST",
                        url: "Avisos.aspx/apagaAviso",
                        data: '{"id_aviso":"' + id_aviso + '", "id_operador":"' + $('#lbloperatorid').html() + '"}',
                        contentType: "application/json; charset=utf-8",
                        dataType: "json",
                        success: function (res) {
                            if (res.d != null) {
                                //$('#search').val(res.d);
                                if (parseInt(res.d) >= 0) {
                                    alertify.message('Aviso apagado com sucesso!');
                                    $('#search').val('');
                                    load();
                                    closePopup();
                                }
        ...
        function updateAviso(id_aviso) {
            if ($('#nrsocioedit').val().trim() == '') {
                alertify.message('O Nº de Sócio tem de ser preenchido!');
                return;
            }

            if ($('#valoredit').val().trim() == '') {
                alertify.message('O Valor tem de ser preenchido!');
                return;
            }
            ...
            if ($('#entradaedit').is(':checked'))
                entrada = "1";

            alertify.confirm('Avisos', 'Tem a certeza que deseja atualizar o aviso?',
                function () {
                    $.ajax({
                        type: "POST",
                        url: "Avisos.aspx/atualizaAviso",
                        data: '{"id_operador":"' + $('#lbloperatorid').html() + '", "nr_socio":"' + $('#nrsocioedit').val().trim() + '", "valor":"' + $('#valoredit').val().trim()
                            + '", "notas":"' + $('#notasedit').val().trim() + '", "id_aviso":"' + id_aviso
                            + '", "data":"' + $('#dataedit').val().trim() + '", "entrada":"' + entrada + '"}',
                        contentType: "application/json; charset=utf-8",
                        dataType: "json",
                        success: function (res) {
                            if (res.d != null) {
                                if (parseInt(res.d) >= 0) {
                                    alertify.message('Aviso atualizado com sucesso!');
                                    $('#search').val('');
                                    load();
                                    closePopup();
                                }
                                else {
                                    alertify.message('Ocorreu um erro ao atualizar o aviso!');
                                }
                            }
                        }
                    });
