<%@ Page Language="C#" AutoEventWireup="true" CodeFile="HorarioWS.aspx.cs" Inherits="HorarioWS" Culture="auto" UICulture="auto" %>
...
<body style="background-color: #FFF !important">
    ...
    <script type="text/javascript">
        ...
        function load() {
            if ($('#selectModalidade').val() == '0') {
                $('#table').html('Por favor, selecione uma modalidade');
            ...
            $.ajax({
                type: "POST",
                url: "HorarioWS.aspx/load",
                data: '{"id_modalidade":"' + $('#selectModalidade').val() + '"}',
        ...
        function createXML() {
            var xml = "<HORARIO><ID_MODALIDADE>";
            xml += $('#selectModalidade').val().trim();
            xml += "</ID_MODALIDADE>";

            for (i = 0; i < parseInt($('#countElements').html()); i++) {
                if ($('#duracao_seg_' + i).val().trim() != '0') {
                    xml += "<AULA><HORA>";
                    xml += $('#hora_' + i).html();
                    xml += "</HORA><DIA>2</DIA><DURACAO>";
                    xml += $('#duracao_seg_' + i).val();
                    xml += "</DURACAO></AULA>";
                }
                if ($('#duracao_ter_' + i).val().trim() != '0') {
                    xml += "<AULA><HORA>";
                    xml += $('#hora_' + i).html();
                    xml += "</HORA><DIA>3</DIA><DURACAO>";
                    xml += $('#duracao_ter_' + i).val();
                    xml += "</DURACAO></AULA>";
                }
                if ($('#duracao_qua_' + i).val().trim() != '0') {
                    xml += "<AULA><HORA>";
                    xml += $('#hora_' + i).html();
                    xml += "</HORA><DIA>4</DIA><DURACAO>";
                    xml += $('#duracao_qua_' + i).val();
                    xml += "</DURACAO></AULA>";
                }
                if ($('#duracao_qui_' + i).val().trim() != '0') {
                    xml += "<AULA><HORA>";
                    xml += $('#hora_' + i).html();
                    xml += "</HORA><DIA>5</DIA><DURACAO>";
                    xml += $('#duracao_qui_' + i).val();
                    xml += "</DURACAO></AULA>";
                }
                if ($('#duracao_sex_' + i).val().trim() != '0') {
                    xml += "<AULA><HORA>";
                    xml += $('#hora_' + i).html();
                    xml += "</HORA><DIA>6</DIA><DURACAO>";
                    xml += $('#duracao_sex_' + i).val();
                    xml += "</DURACAO></AULA>";
                }
                if ($('#duracao_sab_' + i).val().trim() != '0') {
                    xml += "<AULA><HORA>";
                    xml += $('#hora_' + i).html();
                    xml += "</HORA><DIA>SAB</DIA><DURACAO>";
                    xml += $('#duracao_sab_' + i).val();
                    xml += "</DURACAO></AULA>";
                }
            }
        ...
        function updateHorario(xml) {
            $.ajax({
                type: "POST",
                url: "HorarioWS.aspx/updateHorario",
                ...
                success: function (res) {
                    if (res.d != null) {
                        if (parseInt(res.d) >= 0) {
                            alertify.message('Horário atualizado com sucesso!');
                            load();
                        }
                        else {
                            alertify.message('Ocorreu um erro ao atualizar o horário. Por favor, tente novamente!');
                        }
                    }
                }
            });
