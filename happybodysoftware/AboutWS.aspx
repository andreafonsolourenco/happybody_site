<%@ Page Language="C#" AutoEventWireup="true" CodeFile="AboutWS.aspx.cs" Inherits="AboutWS" Culture="auto" UICulture="auto" %>
...
<body style="background-color: #FFF !important">
    <div style="width: 100%;" id="content">
        ...
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" id="divTable">
            <div class="col-lg-8 col-md-8 col-sm-8 col-xs-8" style="margin-bottom: 10px;">
                ...
                <input type="image" src="img/icons/icon_search.png" onclick="loadAbout();" style="float:right; width:auto; height:50px; margin:auto;" />
            </div>
            <div class="col-lg-4 col-md-4 col-sm-4 col-xs-4" style="margin-bottom: 10px;">
                <input type="button" value="Novo" onclick="openNewAbout();" style="float: right; background-color: #4282b5; width: 80%; height: 50px; font-size: 12pt; text-align: center; line-height: 50px; color: #FFFFFF; cursor: pointer; vertical-align: middle; border: none; padding: 0 5px; -moz-border-radius: 2px; -webkit-border-radius: 2px; border-radius: 2px;" />
            </div>
        ...
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="margin-bottom:120px; display:none;" id="divNew">
            <div class='col-lg-6 col-md-6 col-sm-6 col-xs-6' style='text-align:left; margin-top: 5px; margin-bottom: 10px;'>
                <input id='btnSave' value='Guardar' runat='server' type='button' onclick='createAbout();' style='background-color: #4282b5; 
            ...
            <div class='col-lg-6 col-md-6 col-sm-6 col-xs-6' style='text-align:right; margin-top: 5px; margin-bottom: 10px;'>
                <img src='img/icons/icon_close.png' style='cursor:pointer;' onclick='closeNewAbout();'/>
            </div>
            <div class='col-lg-12 col-md-12 col-sm-12 col-xs-12' style='text-align:left; font-family: Noto Sans, sans-serif !important; height:100%'>
                ...
                <div class='col-lg-8 col-md-8 col-sm-8 col-xs-8'>
                    ...
                    <div style='height:50px; width:100%; margin-bottom: 10px'>
                        <input type='text' class='form-control' id='designacaoNew' name='designacaoNew' placeholder='Título' style='width: 100%; margin: auto; height: 100%;' required="required"/>
                    </div>
                    <div style='height:50px; width:100%; margin-bottom: 10px'>
                        <input type='text' class='form-control' id='designacao_enNew' name='designacao_enNew' placeholder='Título Inglês' style='width: 100%; margin: auto; height: 100%;'/>
                    </div>
                    <div style='height:50px; width:100%; margin-bottom: 10px'>
                        <input type='text' class='form-control' id='designacao_esNew' name='designacao_esNew' placeholder='Título Espanhol' style='width: 100%; margin: auto; height: 100%;'/>
                    </div>
                    <div style='height:50px; width:100%; margin-bottom: 10px'>
                        <input type='text' class='form-control' id='designacao_frNew' name='designacao_frNew' placeholder='Título Francês' style='width: 100%; margin: auto; height: 100%;'/>
                    </div>
    ...
    <script type="text/javascript">
        ...
        function openAboutSelected(id, x) {
            $('#lblidselected').html(id);

            $.ajax({
                type: "POST",
                url: "AboutWS.aspx/getAbout",
                data: '{"id":"' + id + '"}',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (res) {
        ...
        function updateInfo() {
            ...
            var textoEn = $('#texto_en').val();
            textoEn = replaceAll(textoEn, "\"", "''");

            var textoEs = $('#texto_es').val();
            textoEs = replaceAll(textoEs, "\"", "''");

            var textoFr = $('#texto_fr').val();
            textoFr = replaceAll(textoFr, "\"", "''");

            $.ajax({
                type: "POST",
                url: "AboutWS.aspx/editar",
                data: '{"id_operador":"' + $('#lbloperatorid').html() + '", "id_about":"' + $('#lblidselected').html() + '", "codigo":"' + $('#codigo').val()
                    + '", "titulo":"' + $('#titulo').val() + '", "titulo_en":"' + $('#titulo_en').val()
                    + '", "titulo_es":"' + $('#titulo_es').val() + '", "titulo_fr":"' + $('#titulo_fr').val()
                    + '", "texto":"' + texto + '", "texto_en":"' + textoEn
                    + '", "texto_es":"' + textoEs + '", "texto_fr":"' + textoFr
                    + '", "ordem":"' + $('#ordem').val() + '"}',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (res) {
        ...
        function createAbout() {
            ...
            if ($('#designacaoNew').val().trim() == '') {
                alertify.message('É necessário definir um título!');
                return;
            }
            ...
            if ($('#ordemNew').val().trim() == '') {
                alertify.message('É necessário definir uma ordem para o artigo!');
                return;
            }
            ...
            var textoEn = $('#texto_enNew').val();
            textoEn = replaceAll(textoEn, "\"", "''");

            var textoEs = $('#texto_esNew').val();
            textoEs = replaceAll(textoEs, "\"", "''");

            var textoFr = $('#texto_frNew').val();
            textoFr = replaceAll(textoFr, "\"", "''");

            $.ajax({
                type: "POST",
                url: "AboutWS.aspx/criar",
                data: '{"id_operador":"' + $('#lbloperatorid').html() + '", "codigo":"' + $('#codigoNew').val()
                    + '", "titulo":"' + $('#designacaoNew').val() + '", "titulo_en":"' + $('#designacao_enNew').val()
                    + '", "titulo_es":"' + $('#designacao_esNew').val() + '", "titulo_fr":"' + $('#designacao_frNew').val()
                    + '", "texto":"' + texto + '", "texto_en":"' + textoEn
                    + '", "texto_es":"' + textoEs + '", "texto_fr":"' + textoFr
                    + '", "ordem":"' + $('#ordemNew').val() + '"}',
                contentType: "application/json; charset=utf-8",
        ...
        function openNewAbout() {
            $('#divTable').fadeOut();
            $('#divNew').fadeIn();
        }

        function closeNewAbout() {
            $('#divTable').fadeIn();
            $('#divNew').fadeOut();
        }

        function deleteAbout() {
            alertify.confirm('Remover elemento do Sobre Nós', 'Tem a certeza que deseja remover o elemento do Sobre Nós?', function () {
                $.ajax({
                    type: "POST",
                    url: "AboutWS.aspx/delete",
